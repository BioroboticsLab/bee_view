...
# (for elevation > 70: very large return values)
...
azimuthMaxFun_1_2 <- approxfun(azimuth$elevation_1_2, azimuth$azimuth_max_1_2, yleft = 300, yright = 300, rule = 2)
azimuthMaxFun_3_4 <- approxfun(azimuth$elevation_3_4, azimuth$azimuth_max_3_4, yleft = -200, yright = -200, rule = 2)
...
p <- ggplot(data.frame(x=azimuth$elevation_1_2, y=azimuth$azimuth_max_1_2), aes(x, y)) +
  stat_function(fun = azimuthMaxFun_1_2) +
  geom_point(colour = "red") +
  geom_point(data = data.frame(x=azimuth$elevation_3_4, y=azimuth$azimuth_max_3_4), colour = "red") +
  stat_function(fun = azimuthMaxFun_3_4) +
  coord_flip(xlim = c(-90, 90), ylim = c(-90, 270)) +
  geom_point(data = data.frame(x = r$y, y = r$x),colour = c("darkblue"), size=0, alpha = .5) +
  coord_flip(xlim = c(-90, 90), ylim = c(-90, 270)) +
  xlab("Elevation in degrees") +
  ylab("Azimuth in degrees") +
  ggtitle("Viewing directions of ommatidia")
p

write.table(file='ommatidia_linear3.csv', x=r, quote = FALSE, sep = ",",row.names = FALSE,col.names = FALSE)
